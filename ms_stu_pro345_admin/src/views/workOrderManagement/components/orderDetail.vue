...
import { orderInfo,orderProgress,orderLog } from '../../../api/order/order';
    export default{
        ...
        methods:{
            //初始化页面
            initPage() {
                // 获取原始时间字符串（如 "2026-05-22T14:50:44"）
                let rawCreateTime = this.$route.query.createTime;
                // 去掉中间的 'T' 替换为空格，若为空则设为空字符串
                this.createTime = rawCreateTime ? rawCreateTime.replace('T', ' ') : '';
